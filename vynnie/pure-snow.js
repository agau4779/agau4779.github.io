...
let baseCSS = ``;
const snowflakeTypes = ["vynnie", "fan", "raquet", "d20", "owl", "elmo", "sharpie", "boba"];
let snowTypeIndex = 0;
...
// Cycle through different snow types
function showSnow() {
  snowTypeIndex = (snowTypeIndex + 1) % snowflakeTypes.length;
  document.body.style.backgroundImage=`url(images/${snowflakeTypes[snowTypeIndex]}-bg.jpg)`;
  createSnow();
}
...
function generateSnow(snowDensity = 200) {
  ...
  for (let i = 0; i < snowDensity; i++) {
    ...
    board.className = snowflakeTypes[snowTypeIndex];
...
function generateSnowCSS(snowDensity = 200) {
  let snowflakeName = snowflakeTypes[snowTypeIndex];
  let rule = baseCSS;
